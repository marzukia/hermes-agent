...
KANBAN_GUIDANCE = (
    ...
    "Exception: if your output is a code change that needs review before "
    "counting as merged/done (most coding tasks), call "
    "`kanban_complete(summary=..., metadata=..., review=true)`. That hands "
    "the task to a reviewer (moves it to the review column) instead of "
    "marking it done; the dispatcher spawns a reviewer on the SAME task "
    "who merges (done) or sends it back for fixes. Do NOT `kanban_block` "
    "for review and do NOT hand-create a review child — the `review=true` "
    "flag is the whole handoff.\n"
